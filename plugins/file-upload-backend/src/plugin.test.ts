...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [fileUploadPlugin],
    });

    await request(server).get('/api/file-upload/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/file-upload/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/file-upload/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/file-upload/todos/${createdTodoItem.id}`)
